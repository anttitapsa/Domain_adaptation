...
class UnetGenerator(nn.Module):
    def __init__(self, input_nc, output_nc, num_downs, ngf=64, norm_layer = nn.BatchNorm2d, use_dropout=False):
        ...
        unet_block = UnetSkipConnectionBLock(output_nc= ngf*8,
                                             inner_nc=ngf*8,
                                             input_nc= None,
                                             submodule= None,
        ...
        unet_block = UnetSkipConnectionBLock(output_nc= ngf*4,
                                             inner_nc=ngf*8,
                                             input_nc= None,
                                             submodule= unet_block,
                                             norm_layer=norm_layer)
        unet_block = UnetSkipConnectionBLock(output_nc= ngf*2,
                                             inner_nc=ngf*4,
                                             input_nc= None,
                                             submodule= unet_block,
                                             norm_layer=norm_layer)
        unet_block = UnetSkipConnectionBLock(output_nc= ngf,
                                             ...
                                             submodule= unet_block,
                                             norm_layer=norm_layer)
        self.model = UnetSkipConnectionBLock(output_nc= output_nc,
                                             inner_nc=ngf,
                                             input_nc= input_nc,
                                             submodule= unet_block,
                                             norm_layer=norm_layer,
...
class UnetSkipConnectionBLock(nn.Module):
    def __inti__(self, outer_nc, inner_nc, input_nc= None, submodule=None, outermost= False, innermost = False, norm_layer=nn.BatchNorm2d, use_dropout= False):
...
class CycleGan(UnetGenerator, NLayerDiscriminator):
    ...
    def backward_G(self):
        ...
        self.loss_idt_A = self.criterionIdt(self.idt_A, self.real_B) * 10 * 0.5
        # G_B should be identity if real_A is fed: ||G_B(A) - A||
        self.idt_B = self.netG_B(self.real_A)
        self.loss_idt_B = self.criterionIdt(self.idt_B, self.real_A) * 10 * 0.5
