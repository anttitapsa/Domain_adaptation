...
def train_loop(net,
    ...
    for epoch in range(epochs):
        net.train()
        loss_training_se = []
        for i, data_source in enumerate(tqdm(source_train_loader)):
                
            # Prepare data
            images = transformer.add_noise_to_images(data_source[0])
...
if __name__ == '__main__':
    LIVECELL_IMG_MB_DIR = os.path.join(data_loader.DATA_DIR, "livecell_magnet_balls", "images")
    LIVECELL_MASK_MB_DIR = os.path.join(data_loader.DATA_DIR, "livecell_magnet_balls", "masks")
    ...
    # Create datasets
    LC_dataset = data_loader.MaskedDataset(LIVECELL_IMG_MB_DIR, LIVECELL_MASK_MB_DIR, length=None, in_memory=False)
